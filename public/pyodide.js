!(function (global, factory) {
    ...
    function initializeFileSystem(Module, config) {
      let stdLibURL;
      (stdLibURL =
        null != config.stdLibURL
          ? config.stdLibURL
          : config.indexURL + "python_stdlib.zip"),
        (function (Module, stdlibURL) {
          const stdlibPromise = loadBinaryFile(stdlibURL);
          Module.preRun.push(() => {
            const pymajor = Module._py_version_major(),
              pyminor = Module._py_version_minor();
            Module.FS.mkdirTree("/lib"),
              Module.FS.mkdirTree(
                `/lib/python${pymajor}.${pyminor}/site-packages`
              ),
              ...
              nativeFSAsync = {
                ...
                reconcile: async (mount, src, dst) => {
                  ...
                  if (
                    ...
                    return;
                  const handles =
                    "remote" === src.type ? src.handles : dst.handles;
                  for (const path of create) {
                    ...
                    if ("local" === dst.type) {
                      ...
                      await nativeFSAsync.storeRemoteEntry(
                        handles,
                        relPath,
                        entry
                      );
    ...
    async function loadPyodide(options = {}) {
      ...
      if ("0.23.2" !== API.version)
        throw new Error(
          `Pyodide version does not match: '0.23.2' <==> '${API.version}'. If you updated the Pyodide version, make sure you also updated the 'indexURL' parameter passed to loadPyodide.`
        );
      ...
      if (
        ...
        "0.23.2" !== API.repodata_info.version)
      )
